...
- **push** publishes your currently selected local files to the dedicated remote folder. After the first push, it tries to reuse unchanged files with WebDAV `COPY` and upload only changes. If the server does not support that operation reliably, push discards the incomplete attempt and uploads the complete file set into a fresh revision instead.
